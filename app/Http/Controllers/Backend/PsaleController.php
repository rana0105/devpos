...
class PsaleController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('backend.psale.create')->withProduct($product)->withPro($pro)->withPacpro($pacpro)->withCustomers($customers)->withType($type)->withProcates($procategories)->withSuppliers($suppliers)->withPacates($pacates)->withShop($shop)->withCopy($copy);
    }
        ...
        if($request->ajax()){
            ...
            if(sizeof($product)>0){
                foreach ($product as $key => $value) {
                    $output .='<div class="productinfo text-left image">
                                <img src="'.asset('upload_file/resize_images/'.$value->image).'" alt="" class="img-responsive" />
